...
function main(){
		...
		if(projectId == "0" || projectId==0){
			eb.abortScript(" You cannot edit the Project Access Members and Project Access Groups fields when creating a Project ",true);
			return;
		}
		...
		var NewConfigurationManagerDG = delta.getNewFieldValue("Configuration Manager DG");
		var OldConfigurationManagerDG = delta.getOldFieldValue("Configuration Manager DG");
		...
		var NewFunctionalSafetyEngineerDG = delta.getNewFieldValue("Functional Safety Engineer DG");
		var OldFunctionalSafetyEngineerDG = delta.getOldFieldValue("Functional Safety Engineer DG");
		...
		var NewFunctionalSafetyLeaderDG = delta.getNewFieldValue("Functional Safety Leader DG");
		var OldFunctionalSafetyLeaderDG = delta.getOldFieldValue("Functional Safety Leader DG");
		...
		var NewHardwareEnginnerLeaderDG = delta.getNewFieldValue("Hardware Enginner Leader DG");
		var OldHardwareEnginnerLeaderDG = delta.getOldFieldValue("Hardware Enginner Leader DG");
		...
		var NewReviewCommitteeLeaderDG = delta.getNewFieldValue("Review Committee Leader DG");
		var OldReviewCommitteeLeaderDG = delta.getOldFieldValue("Review Committee Leader DG");
		...
		for(var i=0 ; i<newList.size() ;i++){
			...
			if(currentNewList != null ){
				...
				if(rightIndex > -1 && leftIndex > -1){
					newStr = currentNewListString.substring(leftIndex+1,rightIndex);
				}else{
			...
			if(currentOldList != null ){
				...
				if(rightIndex > -1 && leftIndex > -1){
					oldStr = currentOldListString.substring(leftIndex+1,rightIndex);
				}else{
			...
			if(isUpdate){
				...
				cmd.addOption(new Packages.com.mks.api.Option("projectmembership",membershipStr));
